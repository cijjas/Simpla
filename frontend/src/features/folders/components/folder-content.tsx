...
export function FolderContent({ folder, onFolderSelect }: FolderContentProps) {
  const {
    ...
    updateFolderNorma,
  } = useFolderNormas(folder?.id || '');
  const { normas: normasWithDetails, loading: normasDetailsLoading } =
  ...
  const folderPath = currentFolder
    ? buildFolderPath(folders, currentFolder.id)
    : [];
  ...
  const _handleRemoveNorma = async (normaId: number, normaTitle: string) => {
    if (
      !confirm(
        `¿Estás seguro de que quieres quitar "${normaTitle}" de esta carpeta?`,
      )
    ) {
      return;
    }
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : 'Error al quitar la norma',
      );
    }
  };
  ...
  const combinedNormas = (folderWithNormas?.normas || []).map(folderNorma => {
    const normaDetails = normasWithDetails.find(
      detail => detail.id === folderNorma.norma.id,
    );
    return {
      ...folderNorma,
      normaDetails,
                ...
                <div className='text-xl flex items-center leading-none font-semibold'>
                  ...
                  <Badge variant='secondary' className='mt-1'>
                    <span className='text-xs text-muted-foreground'>
                      {combinedNormas.length} norma
                      {combinedNormas.length !== 1 ? 's' : ''}
                    </span>
                  </Badge>
                </div>
                {/* Description */}
                        ...
                        <div className='flex items-center gap-3 flex-1 min-w-0'>
                          ...
                          <div className='flex-1 min-w-0'>
                            ...
                            <div className='text-xs text-muted-foreground mt-1'>
                              {normaDetails?.tipoNorma ||
                                folderNorma.norma.tipo_norma ||
                                'Documento'}{' '}
                              •
                              {normaDetails?.jurisdiccion ||
                                folderNorma.norma.jurisdiccion}{' '}
                              • Agregada el{' '}
                              {format(
                                new Date(folderNorma.added_at),
                                'dd/MM/yyyy',
                                { locale: es },
                              )}
                            </div>
