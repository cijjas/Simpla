...
export function AddToFolderDialog({
  ...
  const loadCurrentFolders = useCallback(async () => {
    try {
      ...
      const folderIds = await api.get<string[]>(
        `/api/folders/normas/${normaId}/`,
      );
  ...
  useEffect(() => {
    if (isOpen && normaId) {
      const loadData = async () => {
        ...
        await fetchFolders();
        console.log(
          'AddToFolderDialog: Folders refreshed, current count:',
          folders.length,
        );
  ...
  const flattenFolders = (
    ...
    level = 0,
  ): Array<FolderTreeItem & { level: number }> => {
  ...
  const buildFolderDisplay = () => {
    ...
    console.log(
      'AddToFolderDialog: Building folder display with',
      flatFolders.length,
      'folders:',
      flatFolders.map(f => f.name),
    );
        ...
        >
          <Checkbox
            ...
            onCheckedChange={(checked: boolean) =>
              handleFolderToggle(folder.id, checked)
            }
  ...
  const handleSubmit = async () => {
    try {
      ...
      const foldersToAdd = Array.from(selectedFolderIds).filter(
        id => !originalFolderIds.has(id),
      );
      const foldersToRemove = Array.from(originalFolderIds).filter(
      ...
      for (const folderId of foldersToRemove) {
        try {
          ...
          removedCount++;
          console.log(
            `Successfully removed norma ${normaId} from folder ${folderId}`,
          );
        } catch (error) {
      ...
      for (const folderId of foldersToAdd) {
        try {
          ...
          addedCount++;
          console.log(
            `Successfully added norma ${normaId} to folder ${folderId}`,
          );
        } catch (error) {
      ...
      if (addedCount > 0 && removedCount > 0) {
        toast.success(
          `Norma agregada a ${addedCount} carpeta(s) y removida de ${removedCount} carpeta(s)`,
        );
  ...
  const addCount = Array.from(selectedFolderIds).filter(
    id => !originalFolderIds.has(id),
  ).length;
  const removeCount = Array.from(originalFolderIds).filter(
    id => !selectedFolderIds.has(id),
  ).length;
      ...
      <DialogContent className='max-w-md w-full max-h-[80vh] overflow-hidden'>
        ...
        <div className='space-y-4 w-full overflow-hidden'>
          ...
          {addCount > 0 && (
            <div className='w-full overflow-hidden'>
              <Label htmlFor='notes'>
                Notas para nuevas adiciones (opcional)
              </Label>
          ...
          {hasChanges && (
            <div className='text-sm text-muted-foreground bg-accent/50 p-3 rounded-md'>
              {addCount > 0 && removeCount > 0 && (
                <p>
                  Se agregará a {addCount} carpeta(s) y se quitará de{' '}
                  {removeCount} carpeta(s)
                </p>
              )}
              {addCount > 0 && removeCount === 0 && (
